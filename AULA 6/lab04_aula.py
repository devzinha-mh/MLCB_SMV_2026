...
dados_imobiliaria = [
    ...
    ("Gostaria de cancelar meu contrato de aluguel", "cancelar_contrato"),
    ("Como faço para rescindir o contrato do imóvel?", "cancelar_contrato"),
    ("Quero finalizar o contrato de locação", "cancelar_contrato"),
    ("Preciso de informações sobre o término do meu contrato", "cancelar_contrato"),
    ("Qual o procedimento para cancelar o aluguel?", "cancelar_contrato")
]
...
RESPOSTAS_PADRAO = {
    ...
    "cancelar_contrato": (
        "**Cancelamento de Contrato:** Para solicitar o cancelamento do seu contrato, "
        "por favor, acesse a área do cliente em www.imobiliaria.com/cliente e preencha o formulário de rescisão. "
        "Um de nossos atendentes entrará em contato em breve para dar andamento ao processo."
    )
}
...
def processar_atendimento_sac(mensagem_usuario: str):
    ...
    if confianca >= LIMIAR_CONFIANCA:
        classificacao_status = f" IDENTIFICADO ({intencao_detectada}) | Limiar: {LIMIAR_CONFIANCA*100:.0f}%"
        texto_resposta = RESPOSTAS_PADRAO[intencao_detectada]
    else:
        classificacao_status = f" UNCERTAIN (Fallback Acionado) | Limiar: {LIMIAR_CONFIANCA*100:.0f}%"
        texto_resposta = (
